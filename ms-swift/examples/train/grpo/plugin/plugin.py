...
class MyGRPOReward(ORM):
    
    def extract_labels(self, text: str):
       
        try:
            request_match = re.search(r'Request:\s*(harmful|unharmful|none|harmless)', text, re.IGNORECASE)
            request_label = request_match.group(1).lower() if request_match else None
        except Exception:
            request_label = None

        try:
            response_match = re.search(r'Response:\s*(harmful|unharmful|none|harmless)', text, re.IGNORECASE)
            response_label = response_match.group(1).lower() if response_match else None
        except Exception:
            response_label = None
    ...
    def accuracy_reward(self, res: str, ground_truth: str) -> float:
       
        request_pred, response_pred = self.extract_labels(res)
        request_gt, response_gt = self.extract_labels(ground_truth)

        request_reward = 0.0
        response_reward = 0.0

        if request_pred and request_gt and request_pred == request_gt:
            request_reward = 0.5

        if response_pred and response_gt and response_pred == response_gt:
            response_reward = 0.5

        return request_reward + response_reward

    def conciseness_reward(self, response_length: int, accuracy_score: float) -> float:
        MAX_PENALTY = 0.4
        TARGET_LENGTH = 250
        ...
        length_factor = math.tanh(max(0, excess / TARGET_LENGTH)) 
        return MAX_PENALTY * length_factor
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        """
        rewards = []
        response_token_ids = kwargs['response_token_ids']
        
        for content, sol , response_token_id in zip(completions, solution, response_token_ids):
            token_len = len(response_token_id)

            reward = 0.0
            fmt_score = self.format_reward(content)

            if fmt_score == 1.0:
                acc_score = self.accuracy_reward(content, sol)
                conc_score = self.conciseness_reward(token_len, acc_score)
                reward = acc_score - conc_score
                reward = acc_score
...
# 注册插件
orms['my_grpo_reward'] = MyGRPOReward